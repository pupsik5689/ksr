...
#pragma endregion
		...
		double didx(double x, double i, double l, double r, double v);

		double rk(double x, double i, double h, double l, double r, double v);
		double rk2(double x, double i, double h, double l, double r, double v);
		...
		private: System::Void button2_Click(System::Object^ sender, System::EventArgs^ e) 
		{
			...
			richTextBox1->Text = "";
			double i, i1, i2;  // итог       [0] - vi   [1] - v'i
			...
			chart3->ChartAreas[0]->AxisX->Maximum = (int)(i) + 1;
			chart3->ChartAreas[0]->AxisX->Interval = 0.5;
			chart3->ChartAreas[0]->AxisX->Minimum = -1*(int)(i) - 1;
			...
			res->dataGridView1[2, 0]->Value = i;
			res->dataGridView1[3, 0]->Value = i;
			...
			case 0:
			{
				while ((i < NMax) && (x <= b))
				{
					chart1->Series[0]->Points->AddXY(x, i);
					...
					if ((x <= b) && (x >= b - granica))
					{
						if ((abs((i2 - i1) / 15.) <= eps) && (abs((i2 - i1) / 15.) >= eps / pow(2, 17)))
						{
							i++;
							//s = sqrt(std::max(pow((abs(v2[0] - v1[0]) / 15.) * 16., 2), pow((abs(v2[1] - v1[1]) / 15.) * 16., 2)));
							s = std::max(abs(i2 - i1) / 15., abs(i2 - i1) / 15);
							if (s > maxS)
							{
								maxS = s;
								maxXS = x;
							...
							res->dataGridView1->Rows->Add(i, x, i1, i2, i1, i2, abs(i1 - i2), s, h, delenie, udvoenie);
							i = i1;
							i2 = i;
							...
							if ((abs((i2 - i1) / 15.) < eps / pow(2, 17)) || (abs((i2 - i1) / 15.) < eps / pow(2, 17)))
							{
								i++;
								if (flag == false)
								{
								...
								chart1->Series[0]->Points->AddXY(x, i[0]);
								break;
							}
							else
							{
